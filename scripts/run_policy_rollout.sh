...
if [ "$(uname -s)" = "Darwin" ] && [ "$(/usr/sbin/sysctl -n hw.optional.arm64 2>/dev/null || echo 0)" = "1" ]; then
  if [ "$headless" = false ]; then
    exec /usr/bin/arch -arm64 "$python_path" "$mjpython_path" \
      -m buddy_manipulator.rollout_bc "$@"
  fi
  exec /usr/bin/arch -arm64 "$python_path" -m buddy_manipulator.rollout_bc "$@"
fi

if [ "$(uname -s)" = "Darwin" ] && [ "$headless" = false ]; then
  exec "$mjpython_path" -m buddy_manipulator.rollout_bc "$@"
fi

exec "$python_path" -m buddy_manipulator.rollout_bc "$@"
